...
public class Amount {
     ...
     *            not null and has currency
     * @return
     */
    public Amount add(Amount negotiatedPriceNet) {
        if (negotiatedPriceNet == null) return this;
        if (this.currency == null) currency = negotiatedPriceNet.getCurrency();

        if (sameCurrency(negotiatedPriceNet)) return new Amount(negotiatedPriceNet.getValue().add(this.getValue()), this.currency);
    ...
    public String toString() {
        ...
        String s = "";
        if (value != null) s += DECIMAL_FORMAT.format(value);
        else s += " - ";
        if (currency != null) s += currencyChar;
        else s += " undefined";
    ...
    public Amount multiply(Amount multiplicand) {
        if (this.currency != multiplicand.currency) throw new IllegalArgumentException("Different currencies");
    ...
    public boolean isGreaterZero() {
        ...
        if (this.getValue().compareTo(BigDecimal.ZERO) > 0) return true;
        return false;
    }
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof Amount)) return false;
            ...
            Amount a = (Amount) obj;
            if (a.currency != this.currency) return false;
            if (a.getValue().compareTo(this.getValue()) == 0) return true;
            else return false;
        }
    }
